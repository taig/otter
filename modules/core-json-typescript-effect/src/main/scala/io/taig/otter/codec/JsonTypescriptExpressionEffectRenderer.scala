...
final class JsonTypescriptExpressionEffectRenderer[F[_]: Applicative](
  ...
  def collection[A](json: Json.Read[A] | Json.Write[A]): F[Typescript.Expression] =
    renderer.render(json).map(_.pure[List]).map(Typescript.Expression.Call(name = "Array", _)).map(Schema.apply)

  def dictionary[A](json: Json.Read[A] | Json.Write[A]): F[Typescript.Expression] = renderer
    .render(json)
  ...
  def optional[A](json: Json.Read[A] | Json.Write[A]): F[Typescript.Expression] =
    renderer.render(json).map(_.pure[List]).map(Typescript.Expression.Call(name = "NullOr", _)).map(Schema.apply)

  def record[A](json: Chain[Json.Field.Read[A] | Json.Field.Write[A]]): F[Typescript.Expression] = json.toList
    .traverse:
  ...
  def tuple[A](json: Chain[Json.Read[A] | Json.Write[A]]): F[Typescript.Expression] = json.toList
    .traverse(renderer.render)
    .map(Typescript.Expression.Call(name = "Tuple", _))
    .map(Schema.apply)

  def union[A](json: NonEmptyChain[Json.Read[A] | Json.Write[A]]): F[Typescript.Expression] = json.toList
    .traverse(renderer.render)
