...
final class JsonFTypescriptTypeRenderer[F[_]: Applicative](renderer: Renderer[Json.Write, F[Typescript.Type]])
    extends Renderer[Json.Write, F[Typescript.Type]]:
  override def render[A](json: Json.Write[A]): F[Typescript.Type] = json match
    case json: Json.Constant.Write[A]   => json.encode(JsonPrimitiveTypescriptTypeEncoder).pure
    ...
    case json: Json.Enumeration.Write[A] =>
      Typescript.Type
        .Union(types = json.encode(JsonPrimitiveTypescriptTypeEncoder).toNonEmptyList)
        .pure
    ...
    case json: Json.Record.Write[A]        =>
      json.fields
        .map(_.value)
        .toList
        .traverse(field => renderer.render(field.schema.value).map(tpe => field.name -> tpe))
