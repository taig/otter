...
object Violations:
  extension (violations: Violations)
    ...
    def modifyViolation(f: Violation => Violation): Violations =
  ...
  def one(history: History, violations: NonEmptyChain[Violation]): Violations = NonEmptyMap.one(history, violations)
