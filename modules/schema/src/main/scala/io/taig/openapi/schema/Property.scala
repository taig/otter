...
object Property:
  abstract class Optional[A, B](val value: Option[B]) extends Property[A, Option[B]]:
    @targetName("as")
...
//        copy(property.modify(_.flatMap(validation(_).toOption.map(g))))
    ...
    def empty[A, B]: Property.Optional[A, B] = new Optional[A, B](value = None) {
      override def modify(f: Option[B] => Option[B]): A = ???
    }
