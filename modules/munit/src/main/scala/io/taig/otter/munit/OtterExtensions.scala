...
trait OtterExtensions:
  extension [F[_]: MonadThrow, O](self: F[Validated[Violations, O]])
    def toValid: F[O] = self.flatMap(_.leftMap { violations =>
      new IllegalStateException("Expected Valid, but got Violations", ViolationsException(violations))
    }.liftTo[F])

    def toViolations: F[Violations] = self
      .flatMap(_.swap.leftMap(_ => new IllegalStateException("Expected Invalid, but got Valid")).liftTo[F])
