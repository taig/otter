...
final class ZodRenderer[S[_]: Codec](renderer: Renderer[S, ZodState[String]]) extends Renderer[S, ZodState[String]]:
  override def apply[A](codec: S[A]): ZodState[String] = State: state =>
    codec.metadata(zod) match
      case Some(zod) => (state, zod)
      case None      => renderer(codec).run(initial = state).value

object ZodRenderer:
  def apply[S[_]: Codec](renderer: Renderer[S, ZodState[String]]): Renderer[S, ZodState[String]] =
