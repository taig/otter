...
/** What [[JsonDecoderSuite]] and [[JsonBorerAgreementTest]] structurally cannot reach.
  *
  * Both feed borer a document that started life as text -- the contract through `BorerDoc`, the agreement test through
  * bytes -- so everything they exercise is a `Dom` borer's own JSON parser produced, which is only ever the unsized
  * array and map, and never a byte string, a tag or a non string key. A `Dom` built by hand, or one read from CBOR, can
  * hold all of those, and this module has to answer for them. Every fixture below is therefore built rather than
  * parsed, and every answer is stated absolutely, because there is no document in any other model that corresponds to
  * it to agree with.
  */
object JsonBorerDecoderTest extends JsonDecoderSuite(JsonBorerInterpreter):
  ...
  override protected val extra: List[Spec[TestEnvironment & Scope, Any]] = List(
    suite("borer")(
      ...
      ,
      /** Every violation carries what arrived as a `Data`, and both modules compare those with universal equality, so
        * the ladder has to land on the same rung circe's does. `1.50` reaching `Float` rather than `BigDecimal` is why
        * a non canonical decimal still agrees.
        */
