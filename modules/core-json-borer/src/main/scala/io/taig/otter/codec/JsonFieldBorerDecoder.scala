...
/** Reads a field, reading off its annotation which forms of absence it accepts.
  ...
  */
object JsonFieldBorerDecoder extends Decoder.Remaining[Json.Field.Node, Fields[Dom.Element]]:
  private val lenient: FieldDecoder[Json.Node, Dom.Element] =
    FieldDecoder(JsonBorerDecoder, absent = _.forall(_ == Dom.NullElem))

  private val omitted: FieldDecoder[Json.Node, Dom.Element] = FieldDecoder(JsonBorerDecoder, absent = _.isEmpty)

  private val nulled: FieldDecoder[Json.Node, Dom.Element] =
    FieldDecoder(JsonBorerDecoder, absent = _.exists(_ == Dom.NullElem))

  override def decodeRemaining[R](
      json: Json.Field.Node[Nothing, R],
      values: Fields[Dom.Element]
  ): Validated[Violations, (Fields[Dom.Element], R)] =
