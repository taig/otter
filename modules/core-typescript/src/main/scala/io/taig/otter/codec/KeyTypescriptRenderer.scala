...
object KeyTypescriptRenderer extends Renderer[Key, Typescript.Value]:
  val constant = ConstantTypescriptRenderer[Key](printer = KeyPrinter.Quoted).map(Typescript.Value.apply)
  val enumeration = EnumerationTypescriptRenderer[Key](printer = KeyPrinter.Quoted).map(Typescript.Value.apply)
  val primitive = PrimitiveTypescriptRenderer.map(Typescript.Value.apply)
  ...
  override def render[B](schema: Key[B]): Typescript.Value = schema match
    case Key.Primitive.String(self) => primitive.render(self)
