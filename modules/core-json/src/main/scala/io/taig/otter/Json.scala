...
sealed abstract class Json[A] extends Json.Read[A], Json.Write[A]:
  override def self: Annotation[
    Self.Coerce[Json.Primitive, A] | Self.Collection[Json, A] | Self.Constant[Json.Primitive, A] |
      Self.Dictionary[Json, A] | Self.Enumeration[Json, A] | Self.Optional[Json, A] | Self.Primitive[A] |
...
object Json:
  sealed trait Read[+A]:
    def self: Annotation[
      Self.Coerce.Read[Json.Primitive.Read, A] | Self.Collection.Read[Json.Read, A] |
        Self.Constant.Read[Json.Primitive, A] | Self.Dictionary.Read[Json.Read, A] | Self.Enumeration.Read[Json, A] |
        Self.Optional.Read[Json.Read, A] | Self.Primitive.Read[A] | Self.Record.Read[Json.Field.Read, A] |
  ...
  object Read:
    given Functor[Json.Read]:
      override def map[A, B](json: Json.Read[A])(f: A => B): Json.Read[B] = json match
        case json: Json.Coerce.Read[A]      => json.map(f)
    ...
    given [A] => Annotated[Json.Read[A]]:
      extension (self: Json.Read[A])
        override def lens: (Metadata, Metadata => Json.Read[A]) = self match
          case json: Json.Coerce.Read[A]      => Annotated[Json.Coerce.Read[A]].lens(json)
  ...
  sealed trait Write[-A]:
    def self: Annotation[
      Self.Coerce.Write[Json.Primitive.Write, A] | Self.Collection.Write[Json.Write, A] |
        Self.Constant.Write[Json.Primitive.Write, A] | Self.Dictionary.Write[Json.Write, A] |
        Self.Enumeration.Write[Json.Write, A] | Self.Optional.Write[Json.Write, A] | Self.Primitive.Write[A] |
  ...
  object Write:
    given Contravariant[Json.Write]:
      override def contramap[A, B](json: Json.Write[A])(f: B => A): Write[B] = json match
        case json: Json.Coerce.Write[A]      => json.contramap(f)
    ...
    given [A] => Annotated[Json.Write[A]]:
      extension (self: Json.Write[A])
        override def lens: (Metadata, Metadata => Json.Write[A]) = self match
          case json: Json.Coerce.Write[A]      => Annotated[Json.Coerce.Write[A]].lens(json)
  ...
  final case class Coerce[A](self: Annotation[Self.Coerce[Json.Primitive, A]])
      extends Json[A],
        Json.Coerce.Read[A],
        Json.Coerce.Write[A]
  ...
  object Coerce:
    sealed trait Read[+A] extends Json.Read[A]:
      override def self: Annotation[Self.Coerce.Read[Json.Primitive.Read, A]]

    object Read:
      def apply[A](annotation: Annotation[Self.Coerce.Read[Json.Primitive.Read, A]]): Json.Coerce.Read[A] =
        new Read[A]:
          override def self: Self.Annotation[Self.Coerce.Read[Json.Primitive.Read, A]] = annotation

      given Functor[Json.Coerce.Read] =
        Functor[[a] =>> Annotation[Self.Coerce.Read[Json.Primitive.Read, a]]].imapK([A] =>
          (self: Annotation[Self.Coerce.Read[Json.Primitive.Read, A]]) => Read(self)
        )([A] => (json: Json.Coerce.Read[A]) => json.self)

      given [A] => Annotated[Json.Coerce.Read[A]] =
        Annotated[Annotation[Self.Coerce.Read[Json.Primitive.Read, A]]].imap(Read.apply)(_.self)

      given CoerceOperation.Read[Json.Coerce.Read, Json.Primitive.Read] = CoerceOperation
        .Read[[a] =>> Annotation[Self.Coerce.Read[Json.Primitive.Read, a]], Json.Primitive.Read]
        .imapK([A] => (self: Annotation[Self.Coerce.Read[Json.Primitive.Read, A]]) => Read(self))([A] =>
          (json: Json.Coerce.Read[A]) => json.self
        )

    sealed trait Write[-A] extends Json.Write[A]:
      override def self: Annotation[Self.Coerce.Write[Json.Primitive.Write, A]]

    object Write:
      def apply[A](annotation: Annotation[Self.Coerce.Write[Json.Primitive.Write, A]]): Json.Coerce.Write[A] =
        new Write[A]:
          override def self: Self.Annotation[Self.Coerce.Write[Json.Primitive.Write, A]] = annotation

      given Contravariant[Json.Coerce.Write] =
        Contravariant[[a] =>> Annotation[Self.Coerce.Write[Json.Primitive.Write, a]]].imapK([A] =>
          (annotation: Annotation[Self.Coerce.Write[Json.Primitive.Write, A]]) => Write(annotation)
        )([A] => (json: Json.Coerce.Write[A]) => json.self)

      given [A] => Annotated[Json.Coerce.Write[A]] =
        Annotated[Annotation[Self.Coerce.Write[Json.Primitive.Write, A]]].imap(Write.apply)(_.self)

      given CoerceOperation.Write[Json.Coerce.Write, Json.Primitive.Write] = CoerceOperation
        .Write[[a] =>> Annotation[Self.Coerce.Write[Json.Primitive.Write, a]], Json.Primitive.Write]
        .imapK([A] => (self: Annotation[Self.Coerce.Write[Json.Primitive.Write, A]]) => Write(self))([A] =>
          (json: Json.Coerce.Write[A]) => json.self
        )

    given Invariant[Json.Coerce] = Invariant[[a] =>> Annotation[Self.Coerce[Json.Primitive, a]]].imapK([A] =>
      (annotation: Annotation[Self.Coerce[Json.Primitive, A]]) => Coerce(annotation)
    )([A] => (json: Json.Coerce[A]) => json.self)

    given [A] => Annotated[Json.Coerce[A]] = Annotated[Annotation[Self.Coerce[Json.Primitive, A]]]
      .imap(Coerce.apply)(_.self)

    given CoerceOperation[Json.Coerce, Json.Primitive] =
      CoerceOperation[[a] =>> Annotation[Self.Coerce[Json.Primitive, a]], Json.Primitive].imapK([A] =>
        (annotation: Annotation[Self.Coerce[Json.Primitive, A]]) => Coerce(annotation)
      )([A] => (json: Json.Coerce[A]) => json.self)
  ...
  given [A] => Annotated[Json[A]]:
    extension (self: Json[A])
      override def lens: (Metadata, Metadata => Json[A]) = self match
        case json: Json.Coerce[A]      => Annotated[Json.Coerce[A]].lens(json)
