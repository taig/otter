...
def keyToJson[A](key: Json.Key[A]): Json[A] = key match
  case Json.Key.Constant(self) =>
    Json.Constant(
      self.mapK[Json.Key.Primitive, Json.Primitive]([A] => (self: Json.Key.Primitive[A]) => Json.Primitive(self.self))
    )
  case Json.Key.Enumeration(self) =>
    Json.Enumeration(
      self.mapK[
        Json.Key.Primitive,
        Json.Primitive
      ]([A] => (self: Json.Key.Primitive[A]) => keyPrimitiveToJsonPrimitive(self))
    )
