...
final class EnumerationDecoder[S[_], T](codec: Codec[S, T], render: T => Data.Primitive) extends Decoder[Enumeration[S, *], T]:
  override def decode[A](schema: Enumeration[S, A], value: T): Validated[Violations, A] = schema match
    ...
    case Enumeration.Root(reference, mapping, _) =>
      codec.decode(schema = reference.value, value).andThen: a =>
        mapping
          .unapply(a)
          .toValid:
