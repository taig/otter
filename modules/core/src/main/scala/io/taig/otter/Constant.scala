...
object Constant:
  ...
  object Read:
    ...
    given [F[_]] => ConstantOperation.Read[Constant.Read[F, *], F]:
      ...
      extension [A](fa: Constant.Read[F, A])
        ...
        override def value: Eval[Data] = fa.value
  ...
  object Write:
    final case class Modify[F[_], A, B](self: Constant.Write[F, A], f: B => A) extends Constant.Write[F, B]:
      export self.{value, schema}

    final case class Root[F[_], A](schema: Reference[F, A], reference: Eval[A], render: A => Data) extends Constant.Write[F, A]:
      override def value: Eval[Data] = reference.map(render)
  ...
  final case class Root[F[_], A](schema: Reference[F, A], reference: Eval[A], eq: Eq[A], render: A => Data) extends Constant[F, A]:
    override def value: Eval[Data] = reference.map(render)
