...
final case class Branch[+F <: Data.Any, A](name: String, codec: Eval[Codec[F, A]], metadata: Metadata):
  ...
  def :+[G <: Data.Any, B](branch: Branch[G, B]): Union.Untagged[F | G, Either[A, B]] =
    toUnion :+ branch

  def toUnion: Union.Untagged[F, A] = Union.Untagged.Root(branch = this, metadata = Metadata.Empty)
