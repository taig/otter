...
final class NullableEncoder[S[_], T](encoder: Encoder[S, T], empty: T) extends Encoder[Nullable[S, *], T]:
  override def encode[A](schema: Nullable[S, A], a: A): T = schema match
    ...
    case Nullable.Default(reference, _, _) => encoder.encode(schema = reference.value, a)
    case Nullable.Root(reference, _)       => a.fold(empty)(encoder.encode(schema = reference.value, _))
    case Nullable.Void(_)                  => empty
