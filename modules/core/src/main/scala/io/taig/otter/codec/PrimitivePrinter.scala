...
final class PrimitivePrinter(quotes: Boolean) extends Encoder[Primitive, String]:
  override def encode[A](codec: Primitive[A], a: A): String = codec match
    ...
    case Primitive.Number.BigDecimal(_, _, _)           => a.toPlainString
    case Primitive.Number.BigInteger(_, _, _)           => String.valueOf(a)
    case Primitive.Number.Double(_, _, _)               => String.valueOf(a)
    case Primitive.Number.Float(_, _, _)                => String.valueOf(a)
    case Primitive.Number.Int(_, _, _)                  => String.valueOf(a)
    case Primitive.Number.Long(_, _, _)                 => String.valueOf(a)
    case Primitive.Number.Modify(self, _, g)            => encode(codec = self, g(a))
    case Primitive.String.Modify(self, _, g)            => encode(codec = self, g(a))
    case Primitive.String.Parser(_, _, encode, _, _, _) => apply(encode(a))
    case Primitive.String.Text(_, _, _)                 => apply(a)
