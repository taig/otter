...
abstract class Codec[+O <: Data, A]:
  ...
  /*
  final def nullable(default: A): Nullable[O, A] = Nullable(this, default)
  final def nullable: Nullable[O, Option[A]] = Nullable(this)
   */
...
object Codec:
  ...
  //     self.decode(Data.Nullable(value.map(Data.String.apply)))
  ...
  //   def printNullableObject(a: A): Option[Vector[(String, String)]] = ???
  //   // self.encode(a) match
  //   //   case Data.Null => none
