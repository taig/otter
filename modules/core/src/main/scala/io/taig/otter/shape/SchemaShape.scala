...
trait SchemaShape:
  ...
  object Schema:
    ...
    type Branch[A] = Self.Schema.Branch[?, A]

    object Branch:
      type Of[+S[a] <: Schema[a], A] = Self.Schema.Branch[S, A]
      export Self.Schema.Branch.{apply, unapply}

      type Read[+A] = Self.Schema.Branch.Read[?, A]

      object Read:
        type Of[+S[a] <: Schema.Read[a], A] = Self.Schema.Branch.Read[S, A]
        export Self.Schema.Branch.Read.{apply, unapply}

      type Write[-A] = Self.Schema.Branch.Write[?, A]

      object Write:
        type Of[+S[a] <: Schema.Write[a], A] = Self.Schema.Branch.Write[S, A]
        export Self.Schema.Branch.Write.{apply, unapply}
