...
final class CollectionEncoder[S[_], T](encoder: Encoder[S, T]) extends Encoder[Collection[S, *], Seq[T]]:
  ...
  override def encode[A](schema: Collection[S, A], a: A): Seq[T] = schema match
    case Collection.Indexed(schema, _, _, _, _) => a.map(encoder.encode(schema = schema.value, _))
    case Collection.Linked(schema, _, _, _, _)  => a.map(encoder.encode(schema = schema.value, _))
