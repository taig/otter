...
final class FieldDecoder[S[_], T[_], U](key: Codec[S, String], value: Decoder[T, U]):
  def apply[A](schema: Field[S, T, A], values: List[(String, U)]): Validated[Violations, (List[(String, U)], A)] =
    schema match
      case Field.Modify(self, f, g) => apply(schema = self, values).map(_.map(f))
      ...
      case Field.Optional(self) =>
        ...
        then apply(schema = self, values).map(_.map(_.some))
