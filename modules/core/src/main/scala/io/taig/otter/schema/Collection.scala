...
object Collection:
  ...
  trait Component[+Self[_], -Value[_]](using schema: Collection.Shape[Self, Value]):
    self =>
    ...
    ): Self[List[A]] = self.schema.linked(schema, minimum = minimum.toOption, maximum = maximum.toOption, uniqueItems)
    ...
    final def vector[A](
      ...
      self.schema.indexed(schema, minimum = minimum.toOption, maximum = maximum.toOption, uniqueItems)
