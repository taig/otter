...
final class KeyParser /*(parser: Decoder[Primitive.Value, String], printer: Encoder[Key, String])*/
  ...
  // schema match
  //   case Key.Primitive.String(self)   => parser.decode(schema = self.value, value)
...
object KeyParser:
  ...
  val Unquoted: Decoder[Key, String] =
    ??? // KeyParser(parser = PrimitiveParser.Unquoted, printer = KeyPrinter.Unquoted)
