...
final class KeyParser(parser: Decoder[Primitive, String], printer: Encoder[Key, String]) extends Decoder[Key, String]:
  ...
  override def decode[A](schema: Key[A], value: String): Validated[Violations, A] = schema match
    ...
    case Key.Primitive(self)   => parser.decode(schema = self, value)
