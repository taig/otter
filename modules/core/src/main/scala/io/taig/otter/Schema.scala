...
object Primitive:
  ...
  object Required:
    ...
    final case class Modify[F[_], A, B, C](self: Primitive.Required[A], validation: Validation[A, ?, ?, B], f: B => A)
  ...
  sealed trait Reader[+A] extends Schema.Reader[Nothing, Nothing, A]:
    ...
    def tpe: Type[?]
    override def validate[C](validation: Validation[A, ?, ?, C]): Primitive.Reader[C] = Reader.Modify(this, validation)
  ...
  final case class Modify[A, B](self: Primitive[A], validation: Validation[A, ?, ?, B], f: B => A) extends Primitive[B]:
    export self.tpe
    override def constraints: Chain[Constraint[?]] = self.constraints ++ validation.constraints
...
object Tuple:
  ...
  object Reader:
    ...
    final case class Modify[F[_], A, B, C](self: Tuple.Reader[F, A, B], validation: Validation[B, ?, ?, C])
  ...
  final case class Modify[F[_], A, B, C](self: Tuple[F, A, B], validation: Validation[B, ?, ?, C], f: C => B)
      extends Tuple[F, A, C]:
    override def constraints: Chain[Constraint[?]] = self.constraints ++ validation.constraints
