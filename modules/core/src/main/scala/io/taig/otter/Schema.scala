...
object Schema:
  ...
  object Record:
    ...
    object Write:
      ...
      given [S[a] <: Schema.Write[?, a], A]: Annotated[Schema.Record.Write[S, A]] =
        Annotated[Annotation[RecordBase.Write[Schema.Field.Write[S, *], A]]]
  ...
  object Field:
    ...
    object Read:
      def apply[S[a] <: Schema.Read[?, a], A](annotation: Annotation[FieldBase.Read[S, A]]): Schema.Field.Read[S, A] =
        new Schema.Field.Read[S, A]:
    ...
    object Write:
      ...
      def unapply[S[a] <: Schema.Write[?, a], A](schema: Schema.Field.Write[S, A]): Annotation[FieldBase.Write[S, A]] =
        schema.self
