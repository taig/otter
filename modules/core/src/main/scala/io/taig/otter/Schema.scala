...
object Schema:
  type Of[+S[a] <: Schema[?, a], A] = CoerceBase[S, A] | CollectionBase[S, A] | ConstantBase[S, A] |
    DictionaryBase[S, A] | EnumerationBase[S, A] | NullishBase[S, A] | PrimitiveBase[A] | RecordBase[S, A] |
    TupleBase[S, A] | UnionBase[S, A]
  ...
  object Read:
    type Of[+S[a] <: Schema.Read[?, a], +A] = CoerceBase.Read[S, A] | CollectionBase.Read[S, A] |
      ...
      PrimitiveBase.Read[A] | RecordBase.Read[S, A] | TupleBase.Read[S, A] | UnionBase.Read[S, A]
    ...
    def apply[S[a] <: Schema.Read[?, a], A](annotation: Annotation[Schema.Read.Of[S, A]]): Schema.Read[S, A] =
      annotation.self match
        ...
        case self: RecordBase.Read[S, A]      => Schema.Record.Read(annotation.copy(self = self))
  ...
  object Write:
    type Of[+S[a] <: Schema.Write[?, a], -A] = CoerceBase.Write[S, A] | CollectionBase.Write[S, A] |
      ...
      PrimitiveBase.Write[A] | RecordBase.Write[S, A] | TupleBase.Write[S, A] | UnionBase.Write[S, A]
    ...
    def apply[S[a] <: Schema.Write[?, a], A](annotation: Annotation[Schema.Write.Of[S, A]]): Schema.Write[S, A] =
      annotation.self match
        ...
        case self: RecordBase.Write[S, A]      =>
          Schema.Record.Write(annotation.copy(self = self))
  ...
  sealed abstract class Record[+S[a] <: Schema[?, a], A]
    ...
    override def self: Annotation[RecordBase[S, A]]

  object Record:
    sealed trait Read[+S[a] <: Schema.Read[?, a], +A] extends Schema.Read[S, A]:
      override def self: Annotation[RecordBase.Read[S, A]]

    object Read:
      def apply[S[a] <: Schema.Read[?, a], A](
          annotation: Annotation[RecordBase.Read[S, A]]
      ): Schema.Record.Read[S, A] = new Schema.Record.Read[S, A]:
        override def self: Annotation[RecordBase.Read[S, A]] = annotation

      def unapply[S[a] <: Schema.Read[?, a], A](
          schema: Schema.Record.Read[S, A]
      ): Annotation[RecordBase.Read[S, A]] = schema.self

      given [S[a] <: Schema.Read[?, a], A]: Annotated[Schema.Record.Read[S, A]] =
        Annotated[Annotation[RecordBase.Read[S, A]]].imap(Schema.Record.Read.apply)(_.self)

      given [S[a] <: Schema.Read[?, a]]: Functor[Schema.Record.Read[S, *]] =
        Functor[[a] =>> Annotation[RecordBase.Read[S, a]]].imapK([A] =>
          (annotation: Annotation[RecordBase.Read[S, A]]) => Schema.Record.Read(annotation)
        )([A] => (schema: Schema.Record.Read[S, A]) => schema.self)

      given [S[a] <: Schema.Read[?, a]]: Self.Record.Read[Schema.Record.Read, S] = Self.Record
        .Read[[s[a] <: S[a], a] =>> Annotation[RecordBase.Read[s, a]], S]
    ...
    sealed trait Write[+S[a] <: Schema.Write[?, a], -A] extends Schema.Write[S, A]:
      override def self: Annotation[RecordBase.Write[S, A]]

    object Write:
      def apply[S[a] <: Schema.Write[?, a], A](
          annotation: Annotation[RecordBase.Write[S, A]]
      ): Schema.Record.Write[S, A] = new Schema.Record.Write[S, A]:
        override def self: Annotation[RecordBase.Write[S, A]] = annotation

      def unapply[S[a] <: Schema.Write[?, a], A](
          schema: Schema.Record.Write[S, A]
      ): Annotation[RecordBase.Write[S, A]] = schema.self
      given [S[a] <: Schema.Write[?, a], A]: Annotated[Schema.Record.Write[S, A]] =
        Annotated[Annotation[RecordBase.Write[S, A]]]
          .imap(Schema.Record.Write.apply)(_.self)

      given [S[a] <: Schema.Write[?, a]]: Contravariant[Schema.Record.Write[S, *]] =
        Contravariant[[a] =>> Annotation[RecordBase.Write[S, a]]].imapK([A] =>
          (annotation: Annotation[RecordBase.Write[S, A]]) => Schema.Record.Write(annotation)
    ...
    def apply[S[a] <: Schema[?, a], A](
        annotation: Annotation[RecordBase[S, A]]
    ): Schema.Record[S, A] = new Record[S, A]:
      override def self: Annotation[RecordBase[S, A]] = annotation

    def unapply[S[a] <: Schema[?, a], A](
        schema: Schema.Record[S, A]
    ): Annotation[RecordBase[S, A]] = schema.self

    given [S[a] <: Schema[?, a], A]: Annotated[Schema.Record[S, A]] =
      Annotated[Annotation[RecordBase[S, A]]].imap(Schema.Record.apply)(_.self)

    given [S[a] <: Schema[?, a]]: Invariant[Schema.Record[S, *]] =
      Invariant[[a] =>> Annotation[RecordBase[S, a]]].imapK([A] =>
        (annotation: Annotation[RecordBase[S, A]]) => Schema.Record(annotation)
    ...
    given [S[a] <: Schema[?, a]]: Self.Record[Schema.Record, S] = Self
      ...
      .imapK([s[a] <: S[a], a] => (annotation: Annotation[RecordBase[s, a]]) => Record(annotation))([s[a] <: S[a], a] =>
        (schema: Record[s, a]) => schema.self
      )
  ...
  def apply[S[a] <: Schema[?, a], A](annotation: Annotation[Schema.Of[S, A]]): Schema[S, A] = annotation.self match
    ...
    case self: RecordBase[S, A]      => Schema.Record(annotation.copy(self = self))
