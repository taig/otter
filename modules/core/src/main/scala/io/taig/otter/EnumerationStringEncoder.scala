...
object EnumerationStringEncoder:
  def apply[A](schema: Enumeration.Writer[A], a: A): Option[String] = schema match
    case schema: Enumeration.Required.Writer[A]     => EnumerationRequiredStringEncoder(schema, a).some
  ...
  def optional[A](self: Enumeration.Writer[A], a: Option[A]): Option[String] =
    a.flatMap(EnumerationStringEncoder(self, _))

  def transform[A, B](self: Enumeration.Writer[A], f: B => A, b: B): Option[String] =
