...
trait UnionSchemaInvariant[Self[_], Value[_]] extends SchemaInvariant[Self]:
  ...
  extension [A](self: Self[A])
    ...
    def :+[B](schema: => Value[B]): Self[Either[A, B]] = orElse(lift(schema))
  ...
  extension [A <: Matchable](self: Self[A])
    ...
    inline def |[B <: Matchable](schema: => Value[B]): Self[A | B] = or(lift(schema))

  extension [A](self: => Value[A]) def +:[B](schema: Self[B]): Self[Either[B, A]] = schema.orElse(lift(self))
