...
object Collection:
  sealed abstract class Value[+S[_], A] extends Product, Serializable:
    def validation: Validation[Constraint.Collection, ?]
  ...
  given [Value[_]]: CollectionSchemaInvariant[Collection[Value, *], Value] with
    override def linked[A](
      ...
      value = Value.Linked(schema = Reference.later(schema), validation),
    ...
    override def indexed[A](
      ...
      value = Value.Indexed(schema = Reference.later(schema), validation),
      metadata = Metadata.Empty
    )
