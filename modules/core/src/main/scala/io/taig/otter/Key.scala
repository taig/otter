...
object Key:
  ...
  object Primitive:
    ...
    object Boolean:
      given schema: PrimitiveSchemaInvariant.Boolean[Key.Primitive.Boolean] = PrimitiveSchemaInvariant
        ...
        .imapK([A] => (self: Self.Primitive.Boolean[A]) => Boolean(self))([A] =>
          (key: Key.Primitive.Boolean[A]) => key.self
        )
    ...
    object Number:
      given schema: PrimitiveSchemaInvariant.Number[Key.Primitive.Number] = PrimitiveSchemaInvariant
        ...
        .imapK([A] => (self: Self.Primitive.Number[A]) => Number(self))([A] =>
          (key: Key.Primitive.Number[A]) => key.self
        )
    ...
    object String:
      given schema: PrimitiveSchemaInvariant.String[Key.Primitive.String, Key.Primitive] = PrimitiveSchemaInvariant
        ...
        .imapK([A] => (schema: Self.Primitive.String[Key.Primitive, A]) => String(schema))([A] =>
          (key: Key.Primitive.String[A]) => key.self
        )
  ...
  object Constant:
    given ConstantSchemaInvariant[Key.Constant, Key.Primitive.String] =
      ConstantSchemaInvariant[Self.Constant[Key.Primitive.String, *], Key.Primitive.String]
        .imapK([A] => (schema: Self.Constant[Key.Primitive.String, A]) => Constant(schema))([A] =>
          (key: Key.Constant[A]) => key.self
        )
  ...
  object Enumeration:
    given EnumerationSchemaInvariant[Key.Enumeration, Key.Primitive.String] =
      EnumerationSchemaInvariant[Self.Enumeration[Key.Primitive.String, *], Key.Primitive.String]
        .imapK([A] => (schema: Self.Enumeration[Key.Primitive.String, A]) => Enumeration(schema))([A] =>
          (key: Key.Enumeration[A]) => key.self
        )
  ...
  object Union:
    given UnionSchemaInvariant[Key.Union, Key] = UnionSchemaInvariant[Self.Union[Key, *], Key]
      .imapK([A] => (schema: Self.Union[Key, A]) => Union(schema))([A] => (key: Key.Union[A]) => key.self)

  given SchemaInvariant[Key] with SchemaInvariant.Unionable[Key, Key.Union] with
