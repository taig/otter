...
//     override def encodeRecord(b: B): Vector[(String, O)] = self.encodeRecord(g(b))
...
//     override def encodeRecord(ab: (A, B)): Vector[(String, O | P)] =
...
//   def encodeRecord(a: A): Vector[(String, O)]
...
//     override def encodeRecord(a: Unit): Vector[Nothing] = Vector.empty
//     override def encodeTuple(a: Unit): Vector[Nothing] = Vector.empty

//   def apply[F[+a] <: Data.Nullable[a], O <: Data, A](field: => Field[F, O, A]): Fields[O, A] = new Fields[O, A]:
...
//       ???
//     override def decodeTuple(data: Vector[Data]): Codec.Result[A] = ???
//     // data match
...
//     //       .invalid
//     override def encodeRecord(a: A): Vector[(String, O)] = ??? // field.encode(a).toVector
