...
object TupleBase:
  ...
  object Read:
    ...
    given [S[_]]: Tuple.Read[TupleBase.Read, S] with
      override def apply[T[a] <: S[a], A](schema: Reference[T, A]): TupleBase.Read[T, A] = Root(schema)
      ...
      extension [T[a] <: S[a], A](self: TupleBase.Read[T, A])
        ...
        override def zip[I[a] >: T[a] <: S[a], B](schema: TupleBase.Read[I, B]): TupleBase.Read[I, (A, B)] =
          self.zip(schema)
  ...
  object Write:
    ...
    given [S[_]]: Tuple.Write[TupleBase.Write, S] with
      override def apply[T[a] <: S[a], A](schema: Reference[T, A]): TupleBase.Write[T, A] = Root(schema)
      ...
      extension [T[a] <: S[a], A](self: TupleBase.Write[T, A])
        ...
        override def zip[J[a] >: T[a] <: S[a], B](schema: TupleBase.Write[J, B]): TupleBase.Write[J, (A, B)] =
          self.zip(schema)
  ...
  given [S[_]]: Tuple[TupleBase, S] with
    ...
    override def apply[T[a] <: S[a], A](schema: Reference[T, A]): TupleBase[T, A] = Root(schema)

    extension [T[a] <: S[a], A](self: TupleBase[T, A])
      override def schemas: Chain[Reference[T, ?]] = self.schemas

      override def zip[I[a] >: T[a] <: S[a], B](schema: TupleBase[I, B]): TupleBase[I, (A, B)] = self.zip(schema)
