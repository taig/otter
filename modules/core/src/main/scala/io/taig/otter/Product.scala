...
sealed abstract class Product[+M, A] extends Schema[M, A]:
  ...
  final def zipWith[N >: M, B](merge: (M, N) => N)(
      product: Product[N, B]
  ): Product.Of[self.Of | product.Of, N, (A, B)] = Product.Zip(merge(self.metadata, product.metadata), this, product)
...
object Product:
  ...
  final case class One[C <: Schema[?, A], M, A](metadata: M, codec: C) extends Product[M, A]:
    override type Of = C
  ...
  final case class Zip[M, C1 <: Schema[?, ?], C2 <: Schema[?, ?], A, B](
      metadata: M,
