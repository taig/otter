...
object RecordBase:
  ...
  given [F[+_[a] <: S[a], _], S[_]]: Record[RecordBase, F, S] with
    override def apply[I[a] <: S[a], A](field: F[I, A]): RecordBase[F[I, *], A] = Root(field = Reference.later(field))

    override def empty: RecordBase[F[S, *], Unit] = Empty
