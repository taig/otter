...
object RecordBase:
  ...
  object Read:
    ...
    final case class Root[S[_], A](field: Field[S, A]) extends RecordBase.Read[S, A]:
    ...
    given [S[_]]: Record.Read[RecordBase.Read, S] with
      ...
      override def fields[T[a] <: S[a], A](self: RecordBase.Read[T, A]): Chain[Field[T, ?]] = self.fields
      ...
      override def record[T[a] <: S[a], A](field: Field[T, A]): RecordBase.Read[T, A] = Root(field)

      override def zip[T[a] <: S[a], A, B](
          left: RecordBase.Read[T, A],
          right: RecordBase.Read[T, B]
      ): RecordBase.Read[T, (A, B)] =
        Zip(left, right)
  ...
  object Write:
    ...
    final case class Root[S[_], A](field: Field[S, A]) extends RecordBase.Write[S, A]:
      override def fields: Chain[Field[S, ?]] = Chain.one(field)
    ...
    given [S[_]]: Record.Write[RecordBase.Write, S] with
      ...
      override def fields[T[a] <: S[a], A](self: RecordBase.Write[T, A]): Chain[Field[T, ?]] = self.fields
      ...
      override def record[T[a] <: S[a], A](field: Field[T, A]): RecordBase.Write[T, A] = Root(field)
  ...
  given [S[_]]: Record[RecordBase, S] with
    ...
    override def fields[T[a] <: S[a], A](self: RecordBase[T, A]): Chain[Field[T, ?]] = self.fields
    ...
    override def record[T[a] <: S[a], A](field: Field[T, A]): RecordBase[T, A] = Root(field)
