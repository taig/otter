...
sealed abstract class RecordBase[+S[+_[_], _], +T[_], A] extends RecordBase.Read[S, T, A], RecordBase.Write[S, T, A]:
  ...
  final def zip[U[+s[_], a] >: S[s, a], V[a] >: T[a], B](schema: RecordBase[U, V, B]): RecordBase[U, V, (A, B)] =
...
object RecordBase:
  ...
  object Read:
    ...
    given [S[+_[_], _], T[_]]: cats.Functor[RecordBase.Read[S, T, *]] with
  ...
  object Write:
    ...
    given [S[+_[_], _], T[_]]: cats.Contravariant[RecordBase.Write[S, T, *]] with
  ...
  final case class Modify[S[+_[_], _], T[_], A, B](self: RecordBase[S, T, A], f: A => B, g: B => A)
