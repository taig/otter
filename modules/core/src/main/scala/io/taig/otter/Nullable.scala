...
object Nullable:
  final case class Default[S[_], A](schema: Reference[S, A], default: Eval[A]) extends Nullable[S, A]:
    override def encode[T](encoder: Encoder[S, T]): Option[T] =
      encoder.encode(schema = schema.value, default.value).some
  ...
  final case class Optional[S[_], A](schema: Reference[S, A]) extends Nullable[S, Option[A]]:
    override def encode[T](encoder: Encoder[S, T]): Option[T] = none
