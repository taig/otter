...
object Nullable:
  ...
  given operation[S[_]]: NullableOperation[Nullable[S, *], S] with
    override def nullable[A](value: => S[A]): Nullable[S, Option[A]] = Optional(reference = Reference.later(value))
    override def nullable[A](value: => S[A], default: => A): Nullable[S, A] =
      Default(reference = Reference.later(value), default = Eval.later(default))
