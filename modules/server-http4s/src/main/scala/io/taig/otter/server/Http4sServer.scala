...
final class Http4sServer[F[_]: MonadThrow](f: Http4sApp[F] => Resource[F, Underlying]) extends Server[F]:
  override def start(app: App[F], onError: Throwable => F[Unit]): Resource[F, String] =
    f(toHttp4sApp(app, onError)).map(_.baseUri.show)

object Http4sServer:
  def apply[F[_]: MonadThrow](f: Http4sApp[F] => Resource[F, Underlying]): Server[F] = new Http4sServer[F](f)
