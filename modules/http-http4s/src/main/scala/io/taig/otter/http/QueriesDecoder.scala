...
object QueriesDecoder:
  def withRemainders[A](
    ...
    case Base.Queries.Combine(left, right) => combine(left, right, values)
    case Base.Queries.Empty                => (values, ()).valid
    case Base.Queries.One(query)           => QueryDecoder(query, values)
    case Base.Queries.Transform(self, f)   => transform(self, f, values)

  def combine[A, B](
      left: Queries[A],
      right: Queries[B],
      values: List[(String, Option[String])]
  ): Decoder.Result[Option[String], (List[(String, Option[String])], (A, B))] =
