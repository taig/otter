...
/** Writes the answer an endpoint gave, and the status it gave it under.
  ...
  */
final private[http] class Http4sResultEncoderUnchecked(payload: Http4sPayload[?])
    extends Encoder[Results.Node, Either[Http4sIssue, Http4sWire.Response]]:
  private val results = UnionEncoder(Http4sResultEncoderUnchecked.One(payload))
...
private[http] object Http4sResultEncoderUnchecked:
  ...
  final private[http] class One(payload: Http4sPayload[?])
      extends Encoder[Result.Node, Either[Http4sIssue, Http4sWire.Response]]:
