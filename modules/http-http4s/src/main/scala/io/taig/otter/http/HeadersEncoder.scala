...
object HeadersEncoder:
  def apply[A](headers: Headers.Writer[A], a: A): Http4sHeaders = headers match
    case Base.Headers.Combine(left, right)        => combine(left, right, a)
    case Base.Headers.Empty                       => Http4sHeaders.empty
    case Base.Headers.One(header)                 => one(header, a)
    case Base.Headers.Writer.Combine(left, right) => combine(left, right, a)
    case Base.Headers.Writer.One(header)          => one(header, a)
