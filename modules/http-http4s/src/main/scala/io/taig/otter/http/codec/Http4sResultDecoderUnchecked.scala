...
/** Reads the answer an endpoint gave, choosing the branch the status code names.
  ...
  * is.
  */
final private[http] class Http4sResultDecoderUnchecked(payload: Http4sPayload[?])
    extends Decoder[Results.Node, Http4sWire.Response]:
  private val results = UnionDecoder(Http4sResultDecoderUnchecked.One(payload))
...
private[http] object Http4sResultDecoderUnchecked:
  /** One branch of the union, which answers only under the status it was written for. */
  final private[http] class One(payload: Http4sPayload[?]) extends Decoder[Result.Node, Http4sWire.Response]:
