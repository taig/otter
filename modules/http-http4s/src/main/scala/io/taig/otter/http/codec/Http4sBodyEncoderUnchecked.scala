...
/** Writes a body as the bytes it is, and the media type they go out under.
  ...
  */
final private[http] class Http4sBodyEncoderUnchecked(payload: Http4sPayload[?])
  ...
  private def encode[W](
    ...
    case Body.Value.Whole(mediaType, reference) =>
      payload
        ...
        .toRight(Http4sIssue.Uninterpreted(mediaType))
        .flatMap(_.left.map(Http4sIssue.Encoding(mediaType, _)))
        .map((mediaType, _))
