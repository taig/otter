...
object RequestBodyDecoder:
  def apply[F[_]: Concurrent, A](body: Request.Body[A], value: Http4sEntity[F]): F[Decoder.Result[Any, A]] = body match
    case Http.Request.Body.Singlepart.Strict.Empty =>
      value match
    ...
    case Http.Request.Body.Singlepart.Strict.Binary =>
      value match
    ...
    case Http.Request.Body.Singlepart.Strict.Optional(self) => ???
    case Http.Request.Body.Singlepart.Strict.Apply(parser, decoder, schema) =>
      value match
