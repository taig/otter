...
object BranchJsonEncoder:
  def apply[A](branch: Branch.Writer.Via[Json, A], disciminator: Sum.Discriminator, a: A): JsonObject = branch match
  ...
  def root[A](name: String, schema: Schema.Writer.Via[Json, A], discriminator: Sum.Discriminator, a: A): JsonObject =
    discriminator match
