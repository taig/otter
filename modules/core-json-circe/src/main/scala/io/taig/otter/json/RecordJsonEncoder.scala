...
object RecordJsonEncoder:
  ...
  def apply[A](schema: Record.Writer.Via[Json, A], nulls: Record.Null, a: A): Option[Chain[(String, Json)]] =
    schema match
  ...
  def one[A](field: Field.Writer.Via[Json, A], nulls: Record.Null, a: A): Option[Chain[(String, Json)]] =
    FieldJsonEncoder(field, nulls, a).map(Chain.one)

  def optional[A](self: Record.Writer.Via[Json, A], nulls: Record.Null, a: Option[A]): Option[Chain[(String, Json)]] =
