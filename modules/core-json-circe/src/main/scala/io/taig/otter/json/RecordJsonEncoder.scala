...
object RecordJsonEncoder:
  def apply[A](schema: Record.Via[Json, A], a: A): Option[Chain[(String, Json)]] =
    RecordJsonEncoder(schema, schema.metadata(nulls).getOrElse(Null.Default), a)

  def apply[A](schema: Record.Via[Json, A], nulls: Null, a: A): Option[Chain[(String, Json)]] = schema match
