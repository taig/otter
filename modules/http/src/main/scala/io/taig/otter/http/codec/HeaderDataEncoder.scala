...
object HeaderDataEncoder extends Encoder[Header, Option[Header.Data]]:
  override def encode[A](header: Header[A], a: A): Option[Header.Data] = encode(header = header.self, a)

  def encode[A](header: Header.Value[A], a: A): Option[Header.Data] = header match
    case Header.Value.Root(name, schema) => (name, HeaderValuePrinter.encode(schema = schema.value, a)).some
