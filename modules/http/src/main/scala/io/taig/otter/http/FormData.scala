  ...
  object Dictionary:
    given DictionarySchemaInvariant[FormData.Dictionary, Key, FormData.Schema] =
      DictionarySchemaInvariant[Self.Dictionary[Key, FormData.Schema, *], Key, FormData.Schema]
        .imapK([A] => (schema: Self.Dictionary[Key, FormData.Schema, A]) => Dictionary(schema))([A] =>
          (formData: FormData.Dictionary[A]) => formData.self
        )
  ...
  object Record:
    given RecordSchemaInvariant[FormData.Record, FormData.Field] =
      RecordSchemaInvariant[Self.Record[FormData.Field, *], FormData.Field]
        .imapK([A] => (schema: Self.Record[FormData.Field, A]) => Record(schema))([A] =>
          (formData: FormData.Record[A]) => formData.self
        )
        ...
        given PrimitiveSchemaInvariant.Boolean[FormData.Schema.Primitive.Boolean] =
          PrimitiveSchemaInvariant
            ...
            .imapK([A] => (schema: Self.Primitive.Boolean[A]) => Boolean(schema))([A] =>
              (formData: FormData.Schema.Primitive.Boolean[A]) => formData.self
            )
        ...
        given PrimitiveSchemaInvariant.Number[FormData.Schema.Primitive.Number] =
          PrimitiveSchemaInvariant
            ...
            .imapK([A] => (schema: Self.Primitive.Number[A]) => Number(schema))([A] =>
              (formData: FormData.Schema.Primitive.Number[A]) => formData.self
            )
      ...
      object String:
        given PrimitiveSchemaInvariant.String[FormData.Schema.Primitive.String, FormData.Schema.Primitive] =
          PrimitiveSchemaInvariant
            ...
            .imapK([A] => (schema: Self.Primitive.String[FormData.Schema.Primitive, A]) => String(schema))([A] =>
              (formData: FormData.Schema.Primitive.String[A]) => formData.self
            )
    ...
    object Constant:
      given ConstantSchemaInvariant[FormData.Schema.Constant, FormData.Schema] =
        ConstantSchemaInvariant[Self.Constant[FormData.Schema, *], FormData.Schema]
          .imapK([A] => (schema: Self.Constant[FormData.Schema, A]) => Constant(schema))([A] =>
            (formData: FormData.Schema.Constant[A]) => formData.self
          )
    ...
    object Enumeration:
      given EnumerationSchemaInvariant[FormData.Schema.Enumeration, FormData.Schema] =
        EnumerationSchemaInvariant[Self.Enumeration[FormData.Schema, *], FormData.Schema]
          .imapK([A] => (schema: Self.Enumeration[FormData.Schema, A]) => Enumeration(schema))([A] =>
            (formData: FormData.Schema.Enumeration[A]) => formData.self
          )
    ...
    object Nullable:
      given NullableSchemaInvariant[FormData.Schema.Nullable, FormData.Schema] =
        NullableSchemaInvariant[Self.Nullable[FormData.Schema, *], FormData.Schema]
          .imapK([A] => (schema: Self.Nullable[FormData.Schema, A]) => Nullable(schema))([A] =>
            (formData: FormData.Schema.Nullable[A]) => formData.self
          )
    ...
    object Union:
      given UnionSchemaInvariant[FormData.Schema.Union, FormData.Schema] =
        UnionSchemaInvariant[Self.Union[FormData.Schema, *], FormData.Schema]
          .imapK([A] => (schema: Self.Union[FormData.Schema, A]) => Union(schema))([A] =>
            (formData: FormData.Schema.Union[A]) => formData.self
          )
  ...
  object Field:
    given FieldSchemaInvariant[FormData.Field, Key, FormData.Schema] =
      FieldSchemaInvariant[Self.Field[Key, FormData.Schema, *], Key, FormData.Schema]
        .imapK([A] => (schema: Self.Field[Key, FormData.Schema, A]) => Field(schema))([A] =>
          (formData: FormData.Field[A]) => formData.self
        )
