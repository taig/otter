...
final class QueryValueEncoder(explode: Boolean, style: Query.Style) extends Encoder[Query.Schema, Option[Seq[String]]]:
  override def encode[A](schema: Query.Schema[A], a: A): Option[Seq[String]] = schema match
    case schema: Query.Schema.Atom[A] => Seq(QueryValueAtomPrinter.encode(schema, a)).some
