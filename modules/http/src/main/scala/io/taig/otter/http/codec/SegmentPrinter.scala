...
object SegmentPrinter extends Printer[Segment.Write]:
  override def encode[A](segment: Segment.Write[A], a: A): String = segment match
    ...
    case Segment.Parameter.Root(name, schema)       => SegmentParameterValuePrinter.encode(value = schema.value, a)
    case Segment.Parameter.Write.Modify(self, f)    => encode(segment = self, f(a))
    case Segment.Parameter.Write.Root(name, schema) => SegmentParameterValuePrinter.encode(value = schema.value, a)
