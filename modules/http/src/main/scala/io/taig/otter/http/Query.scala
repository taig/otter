...
abstract class Query[A] extends Query.Read[A], Query.Write[A]:
  final def imap[B](f: A => B)(g: B => A): Query[B] = Query.Modify(self = this, f, g)

  final override def optional: Query[Option[A]] = Query.Optional(self = this)

  final def optional(default: Eval[A]): Query[A] = Query.Default(self = this, value = default)

  override def parameter: Reference[Query.Parameter, ?]

object Query:
  trait Read[+A]:
    final def map[B](f: A => B): Query.Read[B] = Read.Modify(self = this, f)

    def name: String

    def optional: Query.Read[Option[A]] = Read.Optional(self = this)

    def optional[A1 >: A](default: Eval[A1]): Query.Read[A1] = Read.Default(self = this, value = default)

    def parameter: Reference[Query.Parameter, ?]

  object Read:
    final case class Default[A](self: Query.Read[A], value: Eval[A]) extends Query.Read[A]:
      export self.{name, parameter}

    final case class Modify[A, B](self: Query.Read[A], f: A => B) extends Query.Read[B]:
      export self.{name, parameter}

    final case class Optional[A](self: Query.Read[A]) extends Query.Read[Option[A]]:
      export self.{name, parameter}

    final case class Root[A](name: String, parameter: Reference[Query.Parameter, A]) extends Query.Read[A]

    given Functor[Query.Read]:
      override def map[A, B](query: Query.Read[A])(f: A => B): Query.Read[B] = query.map(f)

  trait Write[-A]:
    final def contramap[B](f: B => A): Query.Write[B] = Write.Modify(self = this, f)

    def name: String

    def optional: Query.Write[Option[A]] = Write.Optional(self = this)

    def parameter: Reference[Query.Parameter, ?]

  object Write:
    final case class Modify[A, B](self: Query.Write[A], f: B => A) extends Query.Write[B]:
      export self.{name, parameter}

    final case class Optional[A](self: Query.Write[A]) extends Query.Write[Option[A]]:
      export self.{name, parameter}

    final case class Root[A](name: String, parameter: Reference[Query.Parameter, A]) extends Query.Write[A]

    given Contravariant[Query.Write]:
      override def contramap[A, B](query: Query.Write[A])(f: B => A): Query.Write[B] = query.contramap(f)

  final case class Default[A](self: Query[A], value: Eval[A]) extends Query[A]:
    export self.{name, parameter}

  final case class Modify[A, B](self: Query[A], f: A => B, g: B => A) extends Query[B]:
    export self.{name, parameter}

  final case class Optional[A](self: Query[A]) extends Query[Option[A]]:
    export self.{name, parameter}

  final case class Root[A](name: String, parameter: Reference[Query.Parameter, A]) extends Query[A]

  given Invariant[Query]:
    ...
    object Enumeration:
      ...
      object Read:
        ...
        given [A] => Annotated[Query.Parameter.Enumeration.Read[A]] =
          Annotated[Annotation[Self.Enumeration.Read[Query.Parameter.Primitive.Text.Read, A]]].imap(Read.apply)(_.self)
