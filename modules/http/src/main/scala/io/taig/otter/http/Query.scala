...
object Query:
  ...
  final case class Default[A](name: String, codec: Codec[Data.Primitive, A], metadata: Metadata) extends Query[A]:
    ...
    override def encode(a: A): Query.Value =
      ??? // codec.printNullable(a).fold(Query.Value.None)(Query.Value.Some.apply)
  ...
  final case class Array[A](
    ...
    override def encode(a: A): Query.Value = ???
    // codec.printNullableArray(a) match
