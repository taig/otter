...
trait RequestSyntax:
  def request[A](method: Method, url: Url[A]): Request[Nothing, A] =
    Request(Enrichment(Request.Value.Root(method, url, headers = Headers.Empty).imap((a, _) => a)(a => (a, ()))))

  def request[A, B](method: Method, url: Url[A], headers: Headers[B]): Request[Nothing, (A, B)] =
    Request(Enrichment(Request.Value.Root(method, url, headers).imap((a, b) => (a, b))((a, b) => (a, b))))
  ...
  def request[S[_], A, B](method: Method, url: Url[A], bodies: Bodies[S, B])(using
      merge: Merge[A, B]
  ): Request[S, merge.Out] = Request(Enrichment:
    Request.Value
