...
// final class PathEncoder[F[_]](printer: Printer[F]) extends Encoder[Path.Write[F, *], Chain[String]]:
//   override def encode[A](path: Path.Write[F, A], a: A): Chain[String] = path match
...
//     case Path.Root(segment)              => Chain.one(printer.encode(segment.value, a))
