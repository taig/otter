...
sealed abstract class Results[A]:
  ...
  final def imap[B](f: A => B)(g: B => A): Results[B] = new Results[B]:
    export self.toNev
    override def decode[F[_]](response: Http.Response[F]): Ior[Violations, Option[B]] =
  ...
  final infix def orElse[B](results: Results[B]): Results[Either[A, B]] = new Results[Either[A, B]]:
    ...
    override def decode[F[_]](response: Http.Response[F]): Ior[Violations, Option[Either[A, B]]] =
      self.decode(response) match
        ...
        case Ior.Both(left, None) =>
          results.decode(response) match
            ...
            case Ior.Both(right, b) => Ior.Both(left.combine(right), b.map(_.asRight))
    override def encode[F[_]](ab: Either[A, B]): Http.Response[F] = ab match
  ...
  def decode[F[_]](response: Http.Response[F]): Ior[Violations, Option[A]]
...
object Results:
  ...
  def apply[A](result: Result[A]): Results[A] = new Results[A]:
    ...
    override def decode[F[_]](response: Http.Response[F]): Ior[Violations, Option[A]] =
      result.decode(response).toIor
    override def encode[F[_]](a: A): Http.Response[F] = result.encode(a)
