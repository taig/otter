...
object BodySchema:
  given schema[Self[+_[_], _]](using self: BodySchema[Self]): BodySchema[[s[_], a] =>> Enriched[Self[s, *], a]] with
    override def apply[S[_], A](mediaType: MediaType, schema: => S[A]): Enriched[Self[S, *], A] =
      Enriched(self(mediaType, schema))

    extension [S[_], A](self: Enriched[Self[S, *], A])
      override def mediaType: MediaType = self.self.mediaType
      override def schema: Reference[S, ?] = self.self.schema

    override def schema[S[_]]: Schema[Enriched[Self[S, *], *]] = self
      .schema[S]
      .imapK([A] => (self: Self[S, A]) => Enriched(self))(
        [A] => (self: Enriched[Self[S, *], A]) => self.self
      )
