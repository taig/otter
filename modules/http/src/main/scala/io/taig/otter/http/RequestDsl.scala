...
trait RequestDsl:
  ...
  def request[S, A, B](method: Method, url: Url[A], body: Body[S, B])(using merge: Merge[A, B]): Request[S, merge.Out] =
    Request.Root(method, url, headers = Headers.Empty, body).imap((a, _, b) => (a, b))((a, b) => (a, (), b)).merge

  def request[S, A, B, C](method: Method, url: Url[A], headers: Headers[B], body: Body[S, C]): Request[S, (A, B, C)] =
