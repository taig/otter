...
sealed abstract class Bodies[+S[_], A] extends Product with Serializable:
  def toChain: NonEmptyChain[Reference[S, ?]]
...
object Bodies:
  ...
  final private[otter] case class OrElse[S[_], T[_], A, B](left: Bodies[S, A], right: Bodies[T, B])
    ...
    override def toChain: NonEmptyChain[Reference[S + T, ?]] = left.toChain ++ right.toChain

  final private[otter] case class Root[S[_], A](body: Reference[S, A]) extends Bodies[S, A]:
    override def toChain: NonEmptyChain[Reference[S, ?]] = NonEmptyChain.one(body)
