...
//
//object Url:
//  private[otter] object Empty extends Url[Unit]:
//    override def path: Path[?] = Path.Root
//    override def queries: Queries[?] = Queries.Root
//
//  final private[otter] case class FromPath[A](path: Path[A]) extends Url[A]:
//    override def queries: Queries[?] = Queries.Root
//
//  final private[otter] case class FromQueries[A](queries: Queries[A]) extends Url[A]:
//    override def path: Path[?] = Path.Root
//
