...
object FormDataDecoder extends Decoder[FormData, List[(String, Option[String])]]:
  val dictionary = DictionaryDecoder(key = KeyParser.Unquoted, value = FormDataSchemaParser)
  val record = RecordDecoder(
    field = FieldDecoder(key = KeyCodec.Unquoted, value = FormDataSchemaParser, empty = none[String])
