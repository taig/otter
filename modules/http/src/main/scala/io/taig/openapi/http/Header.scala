...
sealed abstract class Header[A]:
  ...
  def imap[B](f: A => B)(g: B => A): Header[B] // = Header.Modify(this, f, g)
...
object Header:
  final private case class Single[A](name: CIString, schema: Eval[Schema.Value[A]]) extends Header[A]:
    override def imap[B](f: A => B)(g: B => A): Header[B] = copy(schema = schema.map(_.imap(f)(g)))
  ...
  final private case class Optional[A](header: Header[A]) extends Header[Option[A]]:
    ...
    override def imap[B](f: Option[A] => B)(g: B => Option[A]): Header[B] = ??? // copy(header = header.imap(f)(g))
