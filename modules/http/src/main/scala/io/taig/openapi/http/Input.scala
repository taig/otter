...
sealed abstract class Input[A[_[_]]]:
  def method: Method
  def url: Url[?]
  def headers: Headers[?]
  def body: Input.Body[?]

  def encode[F[_]: Concurrent](a: A[F]): Request[F]

object Input:
  sealed abstract class Body[A[_[_]]]:
    def decode[F[_]: Concurrent](body: Request.Body[F]): F[Validated[Violations, A[F]]]
    def encode[F[_]: Concurrent](a: A[F]): Request.Body[F]
  ...
  object Body:
    ...
    abstract class Singlepart[A[_[_]]] extends Body[A]:
      ...
      final override def decode[F[_]: Concurrent](body: Request.Body[F]): F[Validated[Violations, A[F]]] = body match
      ...
      def decode[F[_]: Concurrent](body: Request.Body.Singlepart[F]): F[Validated[Violations, A[F]]]
      override def encode[F[_]: Concurrent](a: A[F]): Request.Body.Singlepart[F]

    object Singlepart:
      object Empty extends Input.Body.Singlepart[Input.Body.Strict[Void]]:
        override def decode[F[_]: Concurrent](body: Request.Body.Singlepart[F]): F[Validated[Violations, Void]] =
          Void.valid.pure[F]
        override def encode[F[_]: Concurrent](a: Void): Request.Body.Singlepart[F] = Request.Body.Singlepart.Empty

      object Strict extends Input.Body.Singlepart[Input.Body.Strict[Array[Byte]]]:
        override def decode[F[_]: Concurrent](body: Request.Body.Singlepart[F]): F[Validated[Violations, Array[Byte]]] =
      ...
      object Streaming extends Input.Body.Singlepart[Input.Body.Streaming[Byte]]:
        ...
        ): F[Validated[Violations, Stream[F, Byte]]] =
          body.data.valid.pure[F]
        override def encode[F[_]: Concurrent](a: Stream[F, Byte]): Request.Body.Singlepart[F] =
          Request.Body.Singlepart(a)

      final private case class Optional[A[_[_]]](body: Body.Singlepart[A])
          extends Input.Body.Singlepart[[F[_]] =>> Option[A[F]]] {
        override def decode[F[_]: Concurrent](
            body: Request.Body.Singlepart[F]
        ): F[Validated[Violations, Option[A[F]]]] =
          // TODO peek into body stream to check whether it's empty
          ???
        override def encode[F[_]: Concurrent](a: Option[A[F]]): Request.Body.Singlepart[F] =
...
////      def decode[F[_]: Concurrent](body: Request.Body.Multipart[F]): Validated[Violations, Effect[F, A]]
  ...
  final private case class Root[A, B, C[_[_]]](method: Method, url: Url[A], headers: Headers[B], body: Body[C])
      extends Input[[F[_]] =>> (A, B, C[F])]:
    override def encode[F[_]: Concurrent](abc: (A, B, C[F])): Request[F] =
      val (path, queries) = url.encode(abc._1)
      Request(method, path, queries, headers.encode(abc._2), body.encode(abc._3))

  transparent inline def apply[A, B, C[_[_]]](
      method: Method,
      url: Url[A],
      headers: Headers[B],
      body: Body[C]
