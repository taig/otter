...
//    override def encode(a: A): Http.Queries = query.encode(a)
  ...
  final private[crock] case class Zip[A, B](left: Queries[A], right: Queries[B]) extends Queries[(A, B)]:
...
//    override def encode(ab: (A, B)): Http.Queries = left.encode(ab._1) merge right.encode(ab._2)
  ...
  final private[crock] case class Modify[A, B](queries: Queries[A], f: A => B, g: B => A) extends Queries[B]:
