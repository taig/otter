...
object TypescriptZodEncoder:
  ...
  private def apply(references: ListMap[String, Typescript], typescript: Typescript): ZodState[TypescriptZod] = typescript match
    ...
    case Typescript.Reference(name) =>
      State: context =>
        if context.stack.contains_(name) then (context, TypescriptZod(typescript, expression = s"z.lazy(() => $name)"))
          ...
          (update.pop.modifyReferences(_.updatedWith(name)(_ => zod.some)), TypescriptZod(typescript, expression = name))
