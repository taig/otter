...
object circe:
  ...
  def toOpenApiObject(json: JsonObject): OpenApi.Object =
    OpenApi.Object(SeqMap.from(json.toIterable.map { case (key, json) => (key, toOpenApi(json)) }))
  ...
  def toJson(openapi: OpenApi): Json = openapi match
    ...
    case OpenApi.Object(values)    => Json.fromFields(values.map { case (key, openapi) => (key, toJson(openapi)) })
