...
object CirceEncoder:
  val schema: Encoder[Schema.Of[Json, *], Option[Json]] = new Encoder:
    override def encode[A](schema: Schema.Of[Json, A], a: A): Option[Json] = schema match
      case schema: Value[A]          => value.encode(schema, a)
      ...
      case schema: AnyValue[Json, A] => anyValue.encode(schema, a)
  ...
  val collection: Encoder[Collection, Option[Chain[Json]]] = ???
//  new Encoder:
...
//      case Schema.Collection.Optional(self)       => a.flatMap(encode(self, _))
  ...
  val dictionary: Encoder[Dictionary, Option[JsonObject]] = ???
//  new Encoder:
...
//        result.append((key.orEmpty, value.getOrElse(Json.Null)))
  ...
  val product: Encoder[Product, Option[List[Json]]] = ???
//  new Encoder:
...
//      case Schema.Product.Optional(self)       => a.foreach(encode(self, _, result))
  ...
  val coproduct: Encoder[Coproduct, Option[Json]] = ???
//  new Encoder:
